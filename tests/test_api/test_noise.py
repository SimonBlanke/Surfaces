...
class TestNoiseIntegration:
    ...
    def test_last_noise_property(self):
        ...
        assert func.last_noise is None

        # After evaluation
        func([0.0, 0.0])
        assert func.last_noise is not None

    def test_noise_property(self):
        """Test that noise property returns the configured noise layer."""
        noise = GaussianNoise(sigma=0.1, seed=42)
        func = SphereFunction(n_dim=2, noise=noise)

        assert func.noise is noise

    def test_no_noise_by_default(self):
        """Test that functions have no noise by default."""
        func = SphereFunction(n_dim=2)

        assert func.noise is None
        assert func.last_noise is None
    ...
    def test_reset_noise(self):
        """Test that reset_noise resets the noise layer state."""
        noise = GaussianNoise(sigma=0.1, seed=42)
        func = SphereFunction(n_dim=2, noise=noise)
    ...
    def test_noise_with_maximize_objective(self):
        ...
        noise = GaussianNoise(sigma=0.1, seed=42)
        func = SphereFunction(n_dim=2, objective="maximize", noise=noise)
    ...
    def test_noise_with_memory_caching(self):
        ...
        noise = GaussianNoise(sigma=0.1, seed=42)
        func = SphereFunction(n_dim=2, noise=noise, memory=True)
