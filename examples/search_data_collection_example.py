...
def main():
    ...
    collection_stats = ml_func.collect_search_data(
        search_space=demo_search_space, batch_size=10, verbose=True
    )
    ...
    for params in test_combinations:
        ...
        print(
            f"Evaluated {params['n_estimators']}/{params['max_depth']}: {result:.4f} ({lookup_time:.6f}s)"
        )

    print(f"Total benchmark time: {total_benchmark_time:.6f} seconds")
    print(
        f"Would have taken ~{timing_stats['average_time'] * len(test_combinations):.4f}s without stored data"
    )
    print()
