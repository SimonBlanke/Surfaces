...
class DatasetRegistry:
    ...
    def _register_default_datasets(self):
        ...
        from ..test_functions.machine_learning.tabular.regression.datasets import diabetes_data
    ...
    def get(self, name: str) -> Callable:
        ...
        if name not in self._datasets:
            raise ValueError(
                f"Dataset '{name}' not found in registry. Available: {list(self._datasets.keys())}"
            )
...
class SearchDataCollector:
    ...
    def collect_search_data(
        ...
        for i, parameters in enumerate(
            self.grid_generator.generate_grid_iterator(processed_search_space)
        ):
            ...
            try:
                ...
                evaluation_errors += 1
                error_msg = (
                    f"Evaluation failed for parameters {parameters}: {type(e).__name__}: {e}"
                )
                if verbose:
            ...
            if len(batch_evaluations) >= batch_size:
                try:
                    ...
                    raise RuntimeError(
                        f"Critical database error during data collection: {error_msg}"
                    ) from e
        ...
        if batch_evaluations:
            try:
                ...
                if verbose:
                    print(f"ERROR: {error_msg}")
                raise RuntimeError(
                    f"Critical database error during final storage: {error_msg}"
                ) from e
        ...
        if evaluation_errors > 0:
            ...
            if error_rate > 50:  # More than 50% failures is concerning
                raise RuntimeError(
                    f"High error rate in evaluations: {warning_msg}. Check your search space and function implementation."
                )
    ...
    def evaluate_with_lookup(
        self, test_function, parameters: Dict[str, Any]
    ) -> Tuple[float, float]:
        """
