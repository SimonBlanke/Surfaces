...
"""CEC 2020 Hybrid Benchmark Functions (F5-F7)."""
...
class HybridFunction2020Base(CEC2020Function):
    """Base class for CEC 2020 hybrid functions.

    Hybrid functions divide the variables into groups and apply different
    base functions to each group.
    """

    # Subclasses define: _proportions, _base_funcs
    _proportions: Tuple[float, ...] = ()

    def _get_group_sizes(self) -> List[int]:
        """Calculate variable group sizes based on proportions."""
        D = self.n_dim
        sizes = []
        remaining = D
    ...
    def _shuffle_and_split(self, z: np.ndarray) -> List[np.ndarray]:
        """Shuffle variables and split into groups."""
...
class HybridFunction1_2020(HybridFunction2020Base):
    """F5: Hybrid Function 1.

    Components:
    - Schwefel's Function (30%)
    - Rastrigin's Function (30%)
    - Elliptic Function (40%)
    """
...
class HybridFunction2_2020(HybridFunction2020Base):
    """F6: Hybrid Function 2.

    Components:
    - Expanded Schaffer's F6 (20%)
    - HGBat (20%)
    - Rosenbrock (30%)
    - Modified Schwefel (30%)
    """
    ...
    def _batch_objective(self, X: ArrayLike) -> ArrayLike:
        ...
        if D3 > 1:
            result += xp.sum(
                100 * (g3_scaled[:, :-1] ** 2 - g3_scaled[:, 1:]) ** 2
                + (g3_scaled[:, :-1] - 1) ** 2,
                axis=1,
            )
        start += sizes[2]
...
class HybridFunction3_2020(HybridFunction2020Base):
    """F7: Hybrid Function 3.

    Components:
    - Expanded Schaffer's F6 (10%)
    - HGBat (20%)
    - Rosenbrock (20%)
    - Modified Schwefel (20%)
    - Elliptic (30%)
    """
...
# Collection of hybrid functions
CEC2020_HYBRID = [
