...
"""CEC 2020 Composition Benchmark Functions (F8-F10)."""
...
class CompositionFunction2020Base(CEC2020Function):
    """Base class for CEC 2020 composition functions.

    Composition functions are weighted sums of multiple base functions,
    each with its own shift and rotation.
    """

    # Subclasses define: _sigmas, _lambdas, _num_funcs
    _sigmas: Tuple[float, ...] = ()
    _lambdas: Tuple[float, ...] = ()
    _num_funcs: int = 0

    def _compute_weights(self, x: np.ndarray) -> np.ndarray:
        """Compute composition weights based on distance to each optimum."""
    ...
    def _batch_compute_weights(self, X: ArrayLike) -> ArrayLike:
        """Compute composition weights for batch."""
...
class CompositionFunction1_2020(CompositionFunction2020Base):
    """F8: Composition Function 1.

    Components:
    - Rastrigin's Function (sigma=10, lambda=1)
    - Griewank's Function (sigma=20, lambda=10)
    - Modified Schwefel's Function (sigma=30, lambda=1)
    """
...
class CompositionFunction2_2020(CompositionFunction2020Base):
    """F9: Composition Function 2.

    Components:
    - Ackley's Function (sigma=10, lambda=10)
    - Elliptic Function (sigma=20, lambda=1e-6)
    - Griewank's Function (sigma=30, lambda=10)
    - Rastrigin's Function (sigma=40, lambda=1)
    """
...
class CompositionFunction3_2020(CompositionFunction2020Base):
    """F10: Composition Function 3.

    Components:
    - Rastrigin's Function (sigma=10, lambda=1)
    - HappyCat Function (sigma=20, lambda=1)
    - Ackley's Function (sigma=30, lambda=10)
    - Discus Function (sigma=40, lambda=1e-6)
    - Rosenbrock's Function (sigma=50, lambda=1)
    """
