    ...
    for func_name, config in ML_FUNCTION_CONFIGS.items():
        ...
        for param1, param2 in config["hyperparameter_pairs"]:
            ...
            fig = plotly_ml_hyperparameter_heatmap(
                ml_func, param1, param2, title=f"{ml_func.name} - {param1} vs {param2}"
            )
            ...
            main_path = os.path.join(
                ml_functions_dir, f"{ml_func._name_}_{param1}_vs_{param2}_heatmap.jpg"
            )
        ...
        for hyperparameter in config["dataset_analyses"]:
            ...
            fig = plotly_dataset_hyperparameter_analysis(
                ml_func, hyperparameter, title=f"{ml_func.name} - Dataset vs {hyperparameter}"
            )

            # Save detailed image
            detailed_path = os.path.join(
                func_detailed_dir, f"dataset_vs_{hyperparameter}_analysis.jpg"
            )
            fig.write_image(detailed_path, format="jpeg", width=1000, height=700)

            # Save main image for README
            main_path = os.path.join(
                ml_functions_dir, f"{ml_func._name_}_dataset_vs_{hyperparameter}_analysis.jpg"
            )
...
def generate_sample_plots():
    ...
    fig1 = plotly_ml_hyperparameter_heatmap(
        ml_func, "n_neighbors", "algorithm", title="Sample: KNN Hyperparameter Analysis"
    )
    fig1.write_image(
        os.path.join(ml_functions_dir, "sample_knn_hyperparams.jpg"),
        format="jpeg",
        width=900,
        height=700,
    )
    ...
    fig2 = plotly_dataset_hyperparameter_analysis(
        ml_func, "n_neighbors", title="Sample: Dataset vs n_neighbors"
    )
    fig2.write_image(
        os.path.join(ml_functions_dir, "sample_knn_datasets.jpg"),
        format="jpeg",
        width=1000,
        height=700,
    )
